/*
 * Gerador de QR Code - biblioteca local, sem dependência de internet/CDN.
 * Baseado no algoritmo open-source "qrcode-generator" (kazuhikoarase), MIT License.
 * Uso: qrcodeGerar(elementoCanvas, "texto ou link", tamanhoEmPixels)
 */
(function (global) {
  var QR8bitByte = function (data) {
    ...
    this.parsedData.length % 3 !== 0 && (this.parsedData.length += 3 - this.parsedData.length % 3);
  };
  ...
  var proto = QRCodeModel.prototype;
  proto.addData = function (data) { this.dataList.push(new QR8bitByte(data)); this.dataCache = null; };
  proto.isDark = function (row, col) {
    if (row < 0 || this.moduleCount <= row || col < 0 || this.moduleCount <= col) throw new Error(row + "," + col);
    return this.modules[row][col];
  };
  proto.getModuleCount = function () { return this.moduleCount; };
  proto.make = function () { this.makeImpl(false, this.getBestMaskPattern()); };
  ...
  QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
    ...
    if (buffer.getLengthInBits() > totalDataCount * 8) throw new Error("texto longo demais para o QR code");
  ...
  QRRSBlock.getRSBlocks = function (typeNumber, errorCorrectLevel) {
    ...
    if (rsBlock == null) throw new Error("erro: tipo/nivel invalido");
  ...
  // API pública simples: desenha o QR code num <canvas>
  function qrcodeGerar(canvas, texto, tamanho) {
    tamanho = tamanho || 200;
    var qr = gerarModelo(texto);
    var count = qr.getModuleCount();
    var cellSize = Math.floor(tamanho / count);
    var margin = Math.floor((tamanho - cellSize * count) / 2);
  ...
  global.qrcodeGerar = qrcodeGerar;
})(window);
